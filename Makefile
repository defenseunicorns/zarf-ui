...
BUILD_ARGS := -s -w -X 'github.com/defenseunicorns/zarf-ui/src/config.UIVersion=$(UI_VERSION)' -X 'github.com/defenseunicorns/zarf/src/config.CLIVersion=$(CLI_VERSION)' -X 'k8s.io/component-base/version.gitVersion=v0.0.0+zarf$(CLI_VERSION)' -X 'k8s.io/component-base/version.gitCommit=$(GIT_SHA)' -X 'k8s.io/component-base/version.buildDate=$(BUILD_DATE)'
.DEFAULT_GOAL := help
...
cve-report: ensure-ui-build-dir ## Create a CVE report for the current project (must `brew install grype` first)
	go run main.go tools sbom packages . -o json | grype -o template -t hack/.templates/grype.tmpl > build/zarf-known-cves.csv
